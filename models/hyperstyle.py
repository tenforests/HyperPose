...
from models.e4e.psp import pSp as e4e
...
class HyperStyle(nn.Module):
    ...
    def forward(self, x,resize=False, input_code=False, randomize_noise=True, return_latents=False,
                return_weight_deltas_and_codes=False, weights_deltas=None, y_hat=None, pose_img=None):
        # false
        if input_code:
            codes = x
        else:
        ...
        # input w codes as style and weights deltas
        # random noise also False
        # get the predict images as yhat and w latent
        ...
        # use this return Gy,w,delta,w code input,pose 
        if return_latents and return_weight_deltas_and_codes:
            return images, result_latent, weights_deltas, codes, pose_img
    ...
    def __get_pretrained_w_encoder(self):
        print("Loading pretrained W encoder...")
        ckpt = torch.load(self.opts.w_encoder_checkpoint_path, map_location='cpu')
        # print(ckpt)
        opts_w_encoder =ckpt['opts']
        # opts_w_encoder = vars(copy.deepcopy(self.opts))
        opts_w_encoder['checkpoint_path'] = self.opts.w_encoder_checkpoint_path
        # opts_w_encoder['encoder_type'] = self.opts.w_encoder_type
        # opts_w_encoder['input_nc'] = 3
        ...
        w_net.cuda()
        return w_net
    def __get_initial_inversion(self, x, resize=True,only_w = False):
        # get initial inversion and reconstruction of batch
        with torch.no_grad():
            if only_w:
                return self.__get_w_inversion_only(x)
            else:
    ...
    def __get_w_inversion_only(self, x):
        if self.w_encoder.training:
            self.w_encoder.eval()
        # x = (x + 1) / 2
        # x = self.e4e_image_transform(x).cuda()
        codes = self.w_encoder.forward(x)
        if codes.ndim == 2:
            codes = codes + self.latent_avg.repeat(codes.shape[0], 1, 1)[:, 0, :]
        else:
