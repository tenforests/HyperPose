...
def tensor2im(image_tensor, imtype=np.uint8, normalize=True):
    if isinstance(image_tensor, list) or (len(image_tensor.size()) == 4 and image_tensor.size()[0] > 1):
        image_numpy = []
...
def crop_wholefaces_ROI(img, xmin_ymin_xmax_ymax):
    xmin,ymin,xmax,ymax = xmin_ymin_xmax_ymax
    return img[..., ymin:ymax,
                    xmin:xmax]
def get_wholefaces_ROI(tensors, xmin_ymin_xmax_ymaxs, opt):
    real_A, real_B, fake_B = tensors
    # Extract wholefaces region of interest around the center.
    real_A_ROI = []
    real_B_ROI = []
    fake_B_ROI = []
    for t in range(xmin_ymin_xmax_ymaxs.shape[0]):
        xmin_ymin_xmax_ymax = xmin_ymin_xmax_ymaxs[t]
        real_A_ROI.append(crop_wholefaces_ROI(real_A[t], xmin_ymin_xmax_ymax))
        real_B_ROI.append(crop_wholefaces_ROI(real_B[t], xmin_ymin_xmax_ymax))
        fake_B_ROI.append(crop_wholefaces_ROI(fake_B[t], xmin_ymin_xmax_ymax))
