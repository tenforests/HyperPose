...
def run_inversion(ref_img,pose_img, net, opts, return_intermediate_results=False):
    y_hat, latent, weights_deltas, codes = None, None, None, None

    if return_intermediate_results:
        results_batch = {idx: [] for idx in range(ref_img.shape[0])}
        results_latent = {idx: [] for idx in range(ref_img.shape[0])}
        results_deltas = {idx: [] for idx in range(ref_img.shape[0])}
    else:
        results_batch, results_latent, results_deltas = None, None, None

    for iter in range(opts.n_iters_per_batch):
        y_hat, latent, weights_deltas, codes, pose_img = net.forward(ref_img,
                                                                    pose_img=pose_img,
