...
class Coach:
    def __init__(self, opts):
        ...
        self.train_dataloader = DataLoader(self.train_dataset,
                                           ...
                                           shuffle=True if opts.seq==False else False,
    ...
    def parse_batch(self, batch):
        ref_img,pose_img,gt_img= None, None, None
        # if isinstance(self.train_dataset, ImagesDataset):
        #     x, y,ref_img = batch
        #     x, y,ref_img = x.to(self.device).float(), y.to(self.device).float(),ref_img.to(self.device).float()
        ...
        #     x = y
        if isinstance(self.train_dataset, MyDataset):
            ref_img,gt_img,pose_img = batch
            ref_img,gt_img,pose_img  = ref_img.to(self.device).float(),gt_img.to(self.device).float(),pose_img.to(self.device).float()
        else:
            raise ValueError("Unsupported dataset type")
        return ref_img,gt_img,pose_img
    ...
    def perform_forward_on_batch(self, batch, train=False):
        ...
        ref_img,gt_img,pose_img = self.parse_batch(batch)
        # yhat is result
        # result = {idx: [] for idx in range(gt_img.shape[0])}
        # x = ref y = gt
        # n_iters_per_batch = 1
        for iter in range(self.opts.n_iters_per_batch):
            ...
            y_hat, latent, weights_deltas, codes, pose_img = self.net.forward(ref_img,
                                                                                 pose_img=pose_img,
            ...
            #     y_hat = y_hat[:, :, 32:224, :]
            # print(gt_img.shape,y_hat.shape)
            loss, cur_loss_dict, id_logs = self.calc_loss(y=gt_img,
                                                          y_hat=y_hat)
            if train:
                loss.backward()
            ...
            #     result[idx].append([y_hat[idx].detach().cpu(), id_logs[idx]['diff_target']])
        return ref_img, gt_img, y_hat, cur_loss_dict, id_logs, pose_img

    def train(self):
        self.net.train()
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(tqdm.tqdm(self.train_dataloader)):

                self.optimizer.zero_grad()

                # x, y, y_hat, loss_dict, id_logs, w_inversion = self.perform_forward_on_batch(batch, train=True)
                ref_img, gt_img, result, loss_dict, id_logs, pose_img = self.perform_forward_on_batch(batch, train=True)
                self.optimizer.step()
                self.train_dataset.set_random_img()
                # Logging related
                if self.global_step % self.opts.image_interval == 0 :
                    self.parse_and_log_images(id_logs, ref_img, gt_img, result, pose_img, title='images/train')
    ...
    def validate(self):
        self.net.eval()
        agg_loss_dict = []
        for batch_idx, batch in enumerate(tqdm.tqdm(self.test_dataloader)):
            if self.opts.max_val_batches is not None and batch_idx > self.opts.max_val_batches:
                break
            with torch.no_grad():
                ref_img, gt_img, result, cur_loss_dict, id_logs, pose_img = self.perform_forward_on_batch(batch)
                self.test_dataset.set_random_img()
            agg_loss_dict.append(cur_loss_dict)

            # Logging related
            self.parse_and_log_images(id_logs, ref_img, gt_img, result, pose_img,
    ...
    def calc_loss(self, y, y_hat):
        ...
        if self.opts.id_lambda > 0:
            loss_id,  id_logs = self.id_loss(y_hat_resized, y_resized)
        ...
        if self.opts.moco_lambda > 0:
            loss_moco, id_logs = self.moco_loss(y_hat, y)
    ...
    def parse_and_log_images(self, id_logs, ref_img, gt_img, result, pose_img, title, subscript=None, display_count=2):
        im_data = []
        display_count = min(ref_img.shape[0], display_count)
        for i in range(display_count):
            # if type(result) == dict:
            #     output_face = [
            #         [common.tensor2im(result[i][iter_idx][0]), result[i][iter_idx][1]]
            #         for iter_idx in range(len(result[i]))
            #     ]
            # else:
            #     output_face = [common.tensor2im(result[i])]
            cur_im_data = {
                'ref_img': common.tensor2im(ref_img[i]),
                'target_img': common.tensor2im(gt_img[i]),
                'output_img': common.tensor2im(result[i]),
                'pose_img': common.tensor2im(pose_img[i])
